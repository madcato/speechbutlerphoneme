...
def load_phonemes(phonemes_file):
    return pd.read_csv(phonemes_file, index_col='word', encoding='utf8')

def phoneme_for(word,column):
    return word_phonemes.loc[word,column]
...
def main(data_directory, output_file):
    ...
    for group in os.listdir(data_directory):
        ...
        for wav_file in glob.glob(speaker_path + "/*.wav"):
            ...
            locale = sfile[3]
            try:
                phoneme = phoneme_for(word,locale)
...
if __name__ == '__main__':
    ...
    parser.add_argument('phonemes_file', type=str,
                        help='Path to output file')
    args = parser.parse_args()
    word_phonemes = load_phonemes(args.phonemes_file)
